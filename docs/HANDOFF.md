...
src/main/ipc.js                          # 插件日志 IPC handler
control-center-preload.js                # 暴露插件日志 API
...
- [ ] **插件 next steps**：
  ...
  - 插件日志持久化/筛选/导出（基础运行日志面板已完成）
  - 插件私有存储清理 UI（基础 `ctx.storage`、key 校验、64KB/插件与 16KB/value 配额已完成）
